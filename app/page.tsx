...
const detectDefaultChildFromScheduleHeader = (text: string): BaseChildKey | null => {
  ...
  if (/של\s*אלין|לאלין\b|לו["׳']?ז\s+של\s*אלין|לו["׳']?ז\s+אלין\b|לוז\s+אלין\b|for\s*alin\b/i.test(head)) {
    return 'alin';
  }
  if (
    /לו["׳']?ז\s+ל\s*רביד|לו["׳']?ז\s+לרביד|לו["׳']?ז\s+רביד\b|לוז\s+רביד\b|לרביד\s+לשבוע|של\s*רביד|לרביד(?:\s|$|[,:.\u05BE])|לו["׳']?ז\s+של\s*רביד|לוז\s+של\s*רביד|הלוז\s+של\s*רביד|for\s*ravid\b/i.test(
      head,
    )
  ) {
    return 'ravid';
...
const insertBulkScheduleLineBreaks = (raw: string): string => {
  ...
  /** "שני 17 עד 1830" — שעה דו-ספרתית בלי נקודתיים (לא היה מפוצל קודם). */
  t = t.replace(
    /([^\n])\s+(?=(?:ראשון|שני|שלישי|רביעי|חמישי|שישי|שבת)\s+\d{1,2}\s+עד\b)/g,
    '$1\n',
  );
  /** "שישי 16 עד 1730" — שעה דו-ספרתית ואז עד (בלי רווח לפני עד אחרי מספר 4 ספרות). */
  t = t.replace(
    /([^\n])\s+(?=(?:ראשון|שני|שלישי|רביעי|חמישי|שישי|שבת)\s+\d{1,2}\s+עד\s+\d)/g,
    '$1\n',
  );
  return t;
};
